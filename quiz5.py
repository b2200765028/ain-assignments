...
def find_list(liste,newlist):
   global oplist1,complist1
   i,j = -1,-1
   while len(newlist) != len(liste):       
      while i < len(operandlist)-1:
         ...
         newlist.append(a)
   return newlist
find_list(operandlist,oplist1 )
find_list(comparisonlist,complist1)
   ...
   try:
         ...
         while z < len(oplist1)-1:
            ...
            if oplist[2] >=0:
               ...
               for j in range(oplist[3],oplist[2]+1):
                  if j%(oplist[0]) == 0 and j%(oplist[1]) !=0:
                     outputlist.append(j)
               assert outputlist == complist
